...
function nodemon(settings) {
  console.log(`>-----------------------------<`.cyan);
  console.log(`>`.cyan + ` [`.blue + `*`.magenta  + `]`.blue + `>`.cyan + ` CODE BY AJAY O S `.magenta );
  console.log(`>`.cyan + ` [`.blue + `*`.magenta  + `]`.blue + `>`.cyan + ` https://github.com/Ajayos `.magenta );
  console.log(`>-----------------------------<`.cyan);
  utils.log.info(`OS  : ${process.platform} `);
  utils.log.info(`path: ${process.cwd()} `);
  utils.log.info(`node: ${process.version} `);
  bus.emit('boot');
  nodemon.reset();
  ...
  config.load(settings, function (config) {
    ...
    config.loaded.map(file => file.replace(cwd, '.')).forEach(file => {utils.log.detail('reading config ' + file)});
    ...
    //if (config.options.restartable) { utils.log.info('to restart at any time, enter `' + config.options.restartable + '`'); }
    ...
    utils.log.info('ext: ' + (config.options.execOptions.ext || '(all)'));
